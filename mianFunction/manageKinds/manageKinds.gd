...
func _on_data_received(_result, response_code, _headers, body):
	...
	match type:
		GetAll:
			var rows =[]
			for value in values:
				var row =[value['kind_id'],value['kind_name']]
				rows.push_back(row)
			$Table.set_rows(rows)
		Delete:
			if values:
...
func _on_deleteConfirmDialog_confirmed():
	if kind_id_to_delete == -1:
		return
	var body = GlobalVar.client.query_string_from_dict({'kind_id' : kind_id_to_delete})
	send_message("deleteKind",body)
...
func _on_AddDialog_addKind(name):
	var body = GlobalVar.client.query_string_from_dict({'kind_name' : name})
	send_message("addKind",body)
